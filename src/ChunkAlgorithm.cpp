...
#define getBlock(x, y, z) (IsValidChunkPosition(x, y, z) ? this_chunk->GetBlock(x, y, z) : wld->GetBlock(chk_base + glm::ivec3(x, y, z)))
#define getSunLight(x, y, z) (IsValidChunkPosition(x, y, z) ? this_chunk->GetSunLight(x, y, z) : wld->GetSunLight(chk_base + glm::ivec3(x, y, z)))
#define getTorchLight(x, y, z) (IsValidChunkPosition(x, y, z) ? this_chunk->GetTorchLight(x, y, z) : wld->GetTorchLight(chk_base + glm::ivec3(x, y, z)))
		...
		for(int index=0; index<CHUNK_INFO_SIZE; ++index)
		{
			...
			for(short face=0; face<6; ++face)
			{
				...
				Block block_neighbour = getBlock(nei[0], nei[1], nei[2]);
					...
					for(it[0] = pos[0]-1; it[0] <= pos[0]+1; ++it[0])
						for(it[1] = pos[1]-1; it[1] <= pos[1]+1; ++it[1])
							for(it[2] = pos[2]-1; it[2] <= pos[2]+1; ++it[2], ++ind)
							{
								...
								sunlight_neighbour[ind] = getSunLight(it[0], it[1], it[2]);
								torchlight_neighbour[ind] = getTorchLight(it[0], it[1], it[2]);
							}
				}
				...
				for (std::size_t j = 0; j < CHUNK_SIZE; ++j) {
					for (std::size_t i = 0; i < CHUNK_SIZE;) {
						...
						if (quad_type) {
							...
							int quad_tex = BlockMethods::GetTexture(quad_type, quad_face);
							...
							bool trans = BlockMethods::IsTransparent(quad_type);
...
#define getLightThreaded(x, y, z) (light[ExXYZ((x), (y), (z))])
		...
		for(int index=0; index<CHUNK_INFO_SIZE; ++index)
		{
			...
			for(short face=0; face<6; ++face)
			{
				...
				Block block_neighbour = getBlockThreaded(nei[0], nei[1], nei[2]);
					...
					for(it[0] = pos[0]-1; it[0] <= pos[0]+1; ++it[0])
						for(it[1] = pos[1]-1; it[1] <= pos[1]+1; ++it[1])
							for(it[2] = pos[2]-1; it[2] <= pos[2]+1; ++it[2], ++ind)
							{
								...
								sunlight_neighbour[ind] = getLightThreaded(it[0], it[1], it[2]) >> 4;
								torchlight_neighbour[ind] = getLightThreaded(it[0], it[1], it[2]) & (uint8_t)0xF;
							}
				}
			...
			for (x[axis] = -1; x[axis] < CHUNK_SIZE;)
			{
				...
				for (x[v] = 0; x[v] < CHUNK_SIZE; ++x[v]) {
					for (x[u] = 0; x[u] < CHUNK_SIZE; ++x[u], ++counter) {
						...
						bool a_out_chunk = x[axis] < 0, b_out_chunk = CHUNK_SIZE - 1 <= x[axis];

						if (!a_out_chunk && ShowFace(a, b))
						{
							mask[counter] = a;
							light_mask[counter] = &lighting_data[XYZ(x[0], x[1], x[2])][axis << 1];

						}
						else if (!b_out_chunk && ShowFace(b, a))
						{
							mask[counter] = -b;
							light_mask[counter] = &lighting_data[
									XYZ(x[0] + q[0], x[1] + q[1], x[2] + q[2])][(axis << 1) | 1];
				...
				for (std::size_t j = 0; j < CHUNK_SIZE; ++j) {
					for (std::size_t i = 0; i < CHUNK_SIZE;) {
						...
						if (quad_type) {
							...
							Face quad_face = (Face) ((axis << 1) | (quad_type <= 0));
